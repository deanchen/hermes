...
(function() {
  var CLIENTS, COMMIT, MAX_COMPLETE, MIN_COMPLETE, PIPELINE, SCORING, STOP_WORDS, args, clients, commitLine, completed, fill_pipeline, finished_processing, fs, hashKeyFields, hashSet, i, index, lazy, path, prefix, prefixClient, prefixScore, problemLines, readLines, redis, sent, startTime, stats, stream, termClient, totalWorkers, uprefixes, workerIndex;
  ...
  commitLine = function(line, i) {
    var client, prefixScores, prefixes;
    if (COMMIT) {
      try {
      ...
      Object.keys(prefixScores).forEach(function(prefix) {
        if ((prefix === "") || uprefixes[prefix]) {
          return;
        }
        return prefixClient.hincrby("prefixes", prefix, 1, function(err, res) {
          if (!(res > 1023)) {
            sent++;
            /* 
            ...
            */
            ...
            return clients[0].zadd(p.phrase, p.score, line.id, function(err) {
              completed++;
