...
(function() {
  var CLIENTS, COMMIT, MAX_COMPLETE, MIN_COMPLETE, PIPELINE, STOP_WORDS, args, clients, commitLine, completed, fill_pipeline, finished_processing, fs, hashKeyFields, hashSet, i, index, lazy, path, prefix, readLines, redis, sent, startTime, stats, stream, termClient, totalWorkers, type, uprefixes, workerIndex;
    ...
    if (COMMIT) {
      ...
      prefix(line).forEach(function(p) {
        ...
        if (!(uprefixes[p] > 1023)) {
          ...
          uprefixes[p]++;
          sent++;
          return clients[0].sadd(p, i, function(err) {
            completed++;
      ...
      sent++;
      hashSet(termClient, i, line, function(err) {
        completed++;
        return fill_pipeline();
      });
      if (i % 10000 === 0) {
